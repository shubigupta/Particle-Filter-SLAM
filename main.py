...
def test_prediction(src_dir, dataset_id=0, split_name='train', log_dir='logs', is_online_fig=False):
    ...
    lidar_odom_fig_path = log_dir + '/lidar_dometry_only_trajectory_'+ split_name + '_' + str(dataset_id) + '.png'# + '.jpg'
    ...
    prediction_fig_path = log_dir + '/prediction_only_trajectory_'+ split_name + '_' + str(dataset_id) + '.png'#'.jpg'
...
def particle_SLAM(src_dir, dataset_id=0, split_name='train', running_mode='test_SLAM', log_dir='logs'):
    ...
    #mov_cov = np.array([[1e-8, 0, 0],[0, 1e-8, 0],[0, 0 , 1e-8]])  #* 0.0000001
    mov_cov = np.array([[9e-5, 0, 0],[0, 9e-5, 0],[0, 0 , 9e-5]])
    ...
    #slam_inc.particles_[:,0] = slam_inc.lidar_.data_[t0]['pose'][0]
    slam_inc.particles_[:,0] = slam_inc.lidar_.data_[0]['pose'][0]
    ...
    #for t in range(t0,num_steps-t0):
        ...
        if (t - t0 + 1)%100 == 0 or t== num_steps- t0 - 1:
        #if (t - t0 + 1)%1 == 0 or t==num_steps-1:
            # Save the result 
